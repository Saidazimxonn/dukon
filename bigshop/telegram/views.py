...
#     template_name = 'teg_messanger.html'
    ...
    #     context_tg = Chanel_Group.objects.filter(category_id=category_id)
class MessangerListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        clinics_filter = Chanel_Group.objects.all()
        ...
        context['categorys_m_list'] = Categorya.objects.all().annotate(count=Count('categor'))
        return context
...
class MessangerDetailView(View):
    ...
    def get(self, request):
        ...
        cate = Chanel_Group.objects.select_related('category').filter(category_id=category_id).in_bulk()
        clinics = list()
...
class MessangerDetailViewInfo(View):
    def get(self, request):
        ...
        info_c = Chanel_Group.objects.select_related('category').filter(id=clinic_info_id).in_bulk()
        info_list = list()
